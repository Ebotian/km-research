...
包内除了插件本体，还有两样**给人看**的东西：`README.md`（包内入口说明：怎么装、装了
先做什么、缺什么）与 `doc/` 的 **Typst 源**（设计方案 + 24 篇调研 + 第九章「实现与计划的
差异」）。**不带编译好的 PDF**——那是 5 MB 级的可重生成物，随包只会让包大十倍；要 PDF
就 `typst compile doc/main.typ`。`verify-zip.sh` 把着这条：包里出现 PDF 即判失败，并且
真的把包内那份文档编译一遍（编译不过说明随包带的是一堆碎片，而不是源）。
...
doc/
  plan/                 设计方案 9 章（1064 行；末章是「实现与计划的差异」）
  sections/             调研附录 24 篇（4565 行）
  main.typ              合稿入口，[Typst](https://typst.app/) 编译出 201 页
```
