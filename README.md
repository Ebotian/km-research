...
- **记录也签。** `save()` 写盘后签名；签名器不可用就**不写**（`4 MISSING`），
  半成品连同可能遗留的旧签名一起清掉——盘上留一份「有 `.sig`、但签名不是这份
  内容」的记录，读的人会以为它可信。
...
```bash
plugin/scripts/build-zip.sh      # -> plugin/dist/open-problem-lab-<版本>.zip（约 408 KB）
...
plugin/                 插件本体
  ...
  lib/                  全部逻辑（5,369 行 / 12 个文件），可类型检查、可单测
...
```bash
plugin/scripts/regress.sh        # 159 项退出码契约回归，夹具自包含
...
`lake` 或没有定点项目时**不跑**（`regress.sh` 那时报 `143 通过 / 0 失败 / 16 跳过`，
环境齐备时报 `159 通过 / 0 失败 / 0 跳过`），`verify-zip.sh` 在同样情形下报
`46 通过 / 0 失败 / 5 跳过`。
...
- **类型检查有盲区**：`bin/` 下的命令没有 `.py` 扩展名（Unix 可执行文件本该如此），
  ...
  1,736 行，只剩 argv 与退出码翻译；逻辑 5,369 行全在 `lib/` 内）。但盲区本身没有消失：
...
**版本 `0.3.0`**（清单里的 `version` 是真源，压缩包名跟着它走）：
...
0.x 里破坏性改动走 minor，所以是 `0.1.0 → 0.2.0`、`0.2.0 → 0.3.0`；`0.2.1` 与
`0.2.2` 都是 patch。`0.3.0` 不是——`0.2.x` 的调用方式在这里**不再照用**：写台账
之前先要有签名器，证据没有签名一律拒收，照旧方式调用会直接失败。这跟「缺验证器时
硬失败」是同一条纪律：写不出一份能验的证据，就不该写出一份像是能的。
