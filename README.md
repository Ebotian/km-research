...
- **证据必须带签名。** `opl-certcheck`、`opl-encode --eval-witness`、`opl-leancheck`
  写出 `--evidence-out` 时当场签出 `<path>.sig`；**没有签名器就不产出无签名的
  证据**，删掉刚写下的那份文件并以 `4 MISSING` 结束。台账加载证据时在**结构检查
  之后**加一道验签：没有签名或核不过一律拒（退出码 `2`，理由点出签名）。
...
定义 sha256 + 评估器 sha256」与当前实验一致的成绩，跳过多少条会打印出来。重建实验时
`--force` 把**旧实验整个目录**归档，是同一条理由的另一半：不归档，旧成绩会继续参与
排名，于是「最好的一条」来自一个已经不存在的实验；而只归档库文件不够——库里的成绩是配着
旧骨架、旧评估器算出来的，留下骨架就等于留下一个成绩与代码对不上的实验，所以归档的是
骨架、评估器、实验定义、运行史、库这一整套，路径 `<lab>/.evolve-bak-<时间戳>`。
...
定，同一秒里重建两次不会覆盖掉上一份备份。切换之后还要把运行史里记着的**暂存期路径**
改写成正式路径，否则「这个数字哪来的」会指向一个已经不存在的目录。这两条都有用例：
`R6-1 复制阶段失败：活实验一个字节都没动`、
`R6-2 切换之后运行史与快照来源仍指得准`。
...
`evaluations.run_dir` 反过来仍是**绝对路径**：那是索引，得能从库里直接导航到某一次运行，
所以搬家时由 `init_lab` 负责把它改写到新位置。`sources` 里按契约属于**人话**而不是路径的
...
```bash
plugin/scripts/build-zip.sh      # -> plugin/dist/open-problem-lab-<版本>.zip（约 415 KB）
...
plugin/                 插件本体
  ...
  lib/                  全部逻辑（5,614 行 / 12 个文件），可类型检查、可单测
...
```bash
plugin/scripts/regress.sh        # 163 项退出码契约回归，夹具自包含
...
`lake` 或没有定点项目时**不跑**（`regress.sh` 那时报 `147 通过 / 0 失败 / 16 跳过`，
环境齐备时报 `163 通过 / 0 失败 / 0 跳过`），`verify-zip.sh` 在同样情形下报
...
- **类型检查有盲区**：`bin/` 下的命令没有 `.py` 扩展名（Unix 可执行文件本该如此），
  ...
  1,742 行，只剩 argv 与退出码翻译；逻辑 5,614 行全在 `lib/` 内）。但盲区本身没有消失：
...
**版本 `0.4.0`**（清单里的 `version` 是真源，压缩包名跟着它走）：
...
`0.2.1` 与 `0.2.2` 都是 patch。`0.3.0` 不是——`0.2.x` 的调用方式在这里**不再照用**：
写台账之前先要有签名器，证据没有签名一律拒收，照旧方式调用会直接失败。`0.4.0` 也不是：
读快照的代码若按绝对路径去找产物，实验目录一搬家就踩空；按 `programs.sqlite.bak-*`
找备份的脚本则会什么都找不到，备份现在是一整个目录。这跟「缺验证器时硬失败」是同一条
纪律：写不出一份能验的证据，就不该写出一份像是能的。
