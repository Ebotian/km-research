...
整目录归档还让索引跟着走：运行史里的 `evaluations.run_dir` 存的是**相对实验目录**的
路径，归档之后每条索引落在归档目录里，不指回活实验。
...
```bash
plugin/scripts/build-zip.sh      # -> plugin/dist/open-problem-lab-<版本>.zip（约 419 KB）
...
plugin/                 插件本体
  ...
  lib/                  全部逻辑（5,640 行 / 12 个文件），可类型检查、可单测
...
```bash
plugin/scripts/regress.sh        # 164 项退出码契约回归，夹具自包含
...
`lake` 或没有定点项目时**不跑**（`regress.sh` 那时报 `148 通过 / 0 失败 / 16 跳过`，
环境齐备时报 `164 通过 / 0 失败 / 0 跳过`），`verify-zip.sh` 在同样情形下报
...
- **类型检查有盲区**：`bin/` 下的命令没有 `.py` 扩展名（Unix 可执行文件本该如此），
  ...
  1,763 行，只剩 argv 与退出码翻译；逻辑 5,640 行全在 `lib/` 内）。但盲区本身没有消失：
...
**版本 `0.5.0`**（清单里的 `version` 是真源，压缩包名跟着它走）：
...
`0.4.0 → 0.5.0`；`0.2.1` 与 `0.2.2` 都是 patch。`0.3.0` 不是——`0.2.x` 的调用方式在这里
**不再照用**：写台账之前先要有签名器，证据没有签名一律拒收，照旧方式调用会直接失败。
...
改用 `opl-evolve-show` 的接口。这跟「缺验证器时硬失败」是同一条纪律：
写不出一份能验的证据，就不该写出一份像是能的。
