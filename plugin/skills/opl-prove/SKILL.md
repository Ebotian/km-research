...
`--evidence-out` 指到一个**已存在**的路径（或它的 `.sig`）会被拒（退出码 `2`）。理由是
台账按**哈希**引用证据：同一路径上的内容被悄悄换掉，那条记录立刻变成「核不过」。所以
证据是**一次写出**的产物，要重做就先删掉旧的那份——那个 `rm` 正是「我知道旧的那份将被
丢弃」的确认——或者换一个路径。在同一处写第二次还有一层更直接的损失：产出流程是
「先写正式文件、再签名」，而签名失败会删掉刚写的那份；于是签名器恰好不可用时重做一次，
先前那份**有效**证据连同它的签名一起没了（实测第一次退出码 `0` 带签名，第二次 `4`，
两份都不在了）。
...
## 记录本身也签（内嵌），核不过的记录只有三条出路

记录与证据的签名**形态不同**，因为它们是两种不同的事务：**证据旁挂**一个 `<path>.sig`，
由验证器命令一次写出，不存在「更新证据」这种事；**记录内嵌**一个顶层 `signature`
字段——记录与它的签名必须待在同一个文件里，才谈得上**一次原子提交**。

写一条记录的顺序是「先算签名 → 整条写进暂存 → 在暂存处自验 → 一次 `os.replace`」。
任何一步没过，正式路径上那份旧记录**一个字节都没动**：失败时看到的是**旧记录原样**，
不是一份「签名核不过」的新记录，也不存在「新记录配旧签名」的中间态。签名器不可用就
**不写**（`MISSING`(4)）——盘上留一份没签名的记录，读的时候分不出它和手写的 JSON。
签的是**去掉 `signature` 之后的规范序列化**，所以只改空白或键序不算改动，任何
**语义**改动都会破签。提交之后顺手删掉可能遗留的旁挂 `.sig`：签名已在文件里，旁挂
那份只会让「这文件有几个签名」变成一道谜题。
