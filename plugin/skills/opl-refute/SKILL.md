...
里面绑定了**规格与见证各自的 sha256**，落盘时还自动签出一份 `<path>.sig`。裸的
`witness.json` 不是证据——它既证明不了「谁复核的」，也证明不了「复核的是这份见证」；
记录靠签名回答前者、靠哈希回答后者。台账只认这种记录。**没有签名器就不产出无签名的
证据**：命令会删掉刚写下的文件、以 `4` 结束，与「找不到 bwrap 就不降级到裸跑」是同
一条纪律；本机还没有密钥就先跑 `$OPL/opl-sign init`。

**同一个路径不许写第二次。** `--evidence-out` 指的路径（或它的 `.sig`）已经存在时，命令
以 `2` 拒绝，不替你覆盖——两个实测过的理由：产出流程是「先写正式文件、再签名」，签名
失败会把刚写下的那份 `discard()` 掉，于是用同一路径写第二次而签名器恰好不可用时，先前
那份**有效**证据连同签名一起被删了（第一次 `0` 且留下签名，第二次 `4`，原证据与原签名
都不在了）；更根本的是**台账按哈希引用证据**，同一路径上的内容被悄悄换掉，那条记录当场
变成「核不过」。所以证据是**一次写出的产物**，要重做只有两条出路——先删掉旧的那份，或
换一个路径：

```bash
# 命令不会替你覆盖；这一下 rm 就是「我知道旧的那份将被丢弃」的确认
rm lab/evidence/C-0001.json lab/evidence/C-0001.json.sig
```

`opl-certcheck`、`opl-encode --eval-witness`、`opl-leancheck` 都按这条办。
...
这条命令的 `--evidence-out` 与 4a 同样只写一次：路径（或它的 `.sig`）已存在就以 `2`
拒绝，要重做先删掉旧的或换一个路径。
...
**证据还必须与结论对得上，而且得是可信来源写出来的**——五项绑定，缺一项就拒
（退出码 `2`）：
...
| 来源 | 证据必须带**可信签名**（`<path>.sig`，由产出它的命令写盘时签出）。缺签名或签名核不过一律拒。**哈希是自证的**——写它的人同时写了它引用的文件，`verdict` 那行字却没有谁验过 |
...
这两条命令都**只写一次**：路径（或它的 `.sig`）已经存在就以 `2` 拒绝，重做要先删掉旧的
那份或换一个路径。

**哈希证明的是「现在这份字节是什么」，证明不了「这句话是谁说的」。** 每次写入重新加载
核的是前者；手写一份自洽的假证据照样能过——写文件的人同时写了被引用的见证，哈希可以
全对，`verdict` 却只是文件里的一行字，没有任何东西验过它。所以 `load_evidence` 在结构
检查之后还要核签名：**没有签名或签名核不过就拒（退出码 `2`），理由里点出是签名的事**，
而不是含混地报一句「证据读不出来」。
...
**签名之前写下的老记录，读得到，改不动。** 记录由 `save()` 写盘时签名，所以「签名
核不过」只有三种来路——手写的、被工具之外的东西改过、从别处拷来的。台账不瞒你：
`opl-conj get` 的 JSON 里会多一个 `"provenance": {"signed": false, …}` 并往 stderr
打印「未经证实」，`opl-conj list` 在行尾标「（未签名/签名核不过）」并给一条汇总。
要改它就两条出路：把签名器配好（`opl-sign init`）之后重跑，或者人工收编——人签字
负责**这份内容**，同时把撑不住的声称降下来：

```bash
$OPL/opl-conj set C-0001 … --adopt --confirmed-by "谁"
```

收编不是照单全收，而是**级联降级**：先记一条 `human_confirmations`（留下是谁收编的、
以及收编时**那份文件的哈希**），再按「档位压回 `empirical`」→「结论退回 `open`」→
「核不过的复核章降成 `UNVERIFIED`」逐级往下压。每压一步都重新校验整条记录、把这一步
写进 `history`、在 stderr 打印一句「降级：…」；三步走完仍不自洽就如实抛错——那是别的
问题，收编不替它兜底。人签的是「这份内容我看过」，不是「这些结论因此成立」，这正是
收编与「顺手改一改」的分界。签名正常时给 `--adopt` 会被拒（退出码 `2`）：静默忽略
一个你明确给的开关，会让你以为自己刚做过什么。
